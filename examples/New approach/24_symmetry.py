...
from compas_singular.framefield.background import BackgroundMesh         # noqa: E402
from compas_singular.framefield.constraints import from_boundary         # noqa: E402
from compas_singular.framefield.constraints import from_curves           # noqa: E402
from compas_singular.framefield.decomposition import FieldDecomposition  # noqa: E402
from compas_singular.framefield.field import CrossField                  # noqa: E402
...
from compas_singular.framefield import symmetry as sym                   # noqa: E402
...
def build(outer, holes, guides, kwargs, spacing, steps):
    """One decomposition, with ``steps`` of the symmetry handling switched on.

    ``steps=None`` is ``symmetry=None`` -- the pipeline as it was before
    2026-08-14, which is what every "original" row below measures.
    """
    symmetry = None
    if steps is not None:
        symmetry = sym.Symmetry.detect(
            [outer] + list(holes or []) + list(guides or []))
        symmetry.steps = tuple(steps)
    return FieldDecomposition.from_boundary(
        outer, holes, guides=guides, target_length=spacing, symmetry=symmetry,
        **kwargs)
...
def part1():
    ...
    print()
    import compas_singular.framefield.background as bgmod
    ...
    print('    Each fix, added to the one above it. Note what row 1 is NOT: the')
    print('    from_curves curve-order fix is a plain bug fix and is applied')
    print('    unconditionally, so it is in every row including "symmetry off".')
    print('    That is why the "+ equivariant guides" row adds nothing here and')
    print('    why row 1 is not the pre-adoption number -- for those, see the')
    print('    table in symmetry.py, or run this against the copy in')
    print('    compas_singular/backup_2026-08-14_pre_symmetry.')
    print()
    rows = []
    for label, steps in (('symmetry off', None),
                         ...
                         ('+ symmetric clustering', ALL_STEPS)):
