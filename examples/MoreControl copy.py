...
point_features = []
polyline_features = [Point(2.5,3.5,0), Point(-0.5,3.5,0)]
polyline_features = densify(polyline_features, 5)
polyline_features = [to_coordinates(polyline_features)]  # one feature curve, as a list of point coords
...
viewer.scene.add(dense_mesh.copy(), name="before")
inp = input("Strip to delete: ")
delete_strips(dense_mesh, [int(inp)])
m=coarse_mesh.copy()
delete_strips(m, [int(inp)])
viewer.scene.add(dense_mesh, name="after")
