...
> other `compas_singular` subpackage. `CMD_start.ensure_paths()` still inserts `SINGULAR_SRC`
...
`rhino/grasshopper.py`, `rhino/constraints/*`, `rhino/geometry/*`.
...
`compas_topology` commands (`CMD_start.py`, `CMD_symmetry_report.py`, `coarse_edit.py`); the three
...
dense  = coarse.densification(edges_to_curves=..., field=...)
```
...
`CMD_coarse_mesh` bakes the layout with `rs.AddMesh`, and a Rhino mesh is
...
* **The field — already solved.** `CMD_start.get_decomposition()` re-solves and
...
* **`edges_to_curves` — the only gap, and it is recomputation, not storage.**
  ...
  `Skeleton::Polylines` — which is exactly what `CMD_coarse_mesh` already does
  after its own bake. The work is to call it in step 6 as well.

#### The change to `CMD_quad_mesh` — **done 2026-09-03**

Implemented as below. `CMD_start` gained `cache_directory()` / `cache_path()` and
...
* **`mismatch` has to be given the same resolvers step 3 solved with.** Checking
  ...
  field. It takes `resolve_relax(settings, guides)` and `resolve_symmetry(settings)`.
...
dense = coarse.densification(edges_to_curves=curves, field=field)
```
...
2. **`polyline_features`.** `SkeletonDecomposition.from_boundary` accepts them
   and no layer offers them. (`CMD_coarse_mesh` passes `[]` deliberately — a
   feature curve *cuts* the domain and leaves non-quad faces; see
   `examples/10_curve_features.py`.)
