...
class Tracer(object):
    ...
    singularity_points : dict[int, [x, y, z]], optional
        ...
        which is what it was before this parameter existed. Set by
        ``symmetry.snap_singularities`` to the field's own ``|u|`` minimum: on a
        symmetric domain the centroids of the singular faces are NOT a symmetric
        set even when the field is exact to 2e-16, because the triangle that
        wins the winding tie around a zero is not equivariant -- while the
    ...
    def _singularity_points(self) -> dict[int, list[float]]:
        ...
        centroids -- it has to, or the position chosen here is silently
        discarded when it puts a point back on the front of every trace.
        """
        return {fkey: self.singularity_points.get(fkey)
