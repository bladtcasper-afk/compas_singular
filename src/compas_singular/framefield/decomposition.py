...
class FieldDecomposition(object):
    ...
    def smooth_quad_mesh(self, mesh, **kwargs):
        """**Relax a dense mesh globally**, after :meth:`quad_mesh` has built it.

        Not named ``relax``: that word is already taken on this class by the FIELD
        solve (:meth:`from_boundary`'s ``relax=``), and the two are unrelated.

        ``densify`` solves each patch with its boundaries fixed, so the seams and
        anything the Coons reference already got wrong are out of its reach at any
        stiffness. This is the pass that can reach them. It only keeps a result that
        improves the minimum angle, the maximum angle and the aspect ratio together,
        so a domain with nothing to gain comes back untouched.

        Modifies ``mesh`` in place. See :mod:`framefield.relax` for what is
        constrained and, more to the point, what deliberately is not.

        Returns
        -------
        dict
            The report from :func:`framefield.relax.relax_mesh` -- ``accepted``,
            ``before``, ``after``, ``sliding``, ``pinned``.
        """
        from .relax import relax_mesh

        report = relax_mesh(
            mesh,
            seam_edge=getattr(self, 'densify_stats', {}).get('seam_edge'),
            edges_to_curves=self.edges_to_curves(),
            **kwargs)
        if report['accepted'] is None:
            self.repair_notes.append(
                'RELAX: no setting improved min/max/aspect together; mesh left as densified.')
        return report
