...
def post(verb, args=None, directory=None):
    ...
    request_id = new_id()
    _write_json(
        os.path.join(folder, request_id + REQUEST_SUFFIX),
        {'id': request_id, 'verb': verb, 'args': dict(args or {}),
         'created': time.time()})
    return request_id
...
def wait(request_id, timeout=DEFAULT_TIMEOUT, directory=None):
    ...
    **Never raises and never blocks past** ``timeout``. A deadline reached is a
    ...
    dict
        ...
        ``queued`` so the caller can say WHY nothing came back.
    """
    folder = spool_directory(directory)
