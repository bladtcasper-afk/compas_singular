...
def boundary_corners(loop, limit=pi / 12.0, spacing=None, sharp=SHARP_TURN):
    ...
    # ``typical + limit``.
    out = [i for i, a in turn.items() if a >= sharp]

    candidates = sorted(i for i, a in turn.items() if a < sharp)
    if not candidates:
        return sorted(out)
    if len(candidates) == 1:
    ...
    for prev, i in pairwise(candidates):
        if cum[i] - cum[prev] <= gap:
            runs[-1].append(i)
        else:
            runs.append([i])
    if len(runs) > 1 and (total - cum[candidates[-1]]) + cum[candidates[0]] <= gap:
...
def _cluster(points, tol, loops=None, wall_tol=None, groups=None):
    ...
    return {i: points[j] for i, j in canonical.items()}
