...
def field_densification(coarse, field, tracer, edges_to_curves=None,
    ...
    pole_map = [geometric_key(coarse.vertex_coordinates(pole))
    ...
    for mesh in meshes:
        for fkey in mesh.faces():
            ...
            for u, v in pairwise(fv + fv[:1]):
                gk = geometric_key(mesh.vertex_coordinates(u))
                if gk in pole_map and gk == geometric_key(mesh.vertex_coordinates(v)):
                    face_pole_map[geometric_key(mesh.face_center(fkey))] = gk
                    break
    ...
    for fkey in dense.faces():
        gk = geometric_key(dense.face_center(fkey))
        if gk in face_pole_map:
            for vkey in dense.face_vertices(fkey):
                if geometric_key(dense.vertex_coordinates(vkey)) == face_pole_map[gk]:
                    face_pole[fkey] = vkey
                    break
