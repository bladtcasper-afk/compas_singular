...
Delaunay triangulation of the 2D (XY-planar) design domain. It is not a
*constrained* Delaunay: features survive as edges because they are sampled as
densely as everything else (see §5).
...
5. `quadrangulate_polygonal_faces()` — seam propagation: a face with more than
   four corners, left where a curve feature's two sides disagree, is split by
   carrying the discrepancy across to a wall (§5). `repair_polygonal_faces()` is
   the fallback for whatever that cannot resolve.
6. `split_quads_with_poles(points)` / `store_pole_data(points)` — for every
...
| off the boundary | a singularity — two-valent when one singular face touches the tip, a pole when several do |
...
**Working for open features that land on walls, meet other features, or end
free.** All 15 benchmark cases in `examples/000_testing.py` are all-quad with no
fallback repair. Every one keeps all its domain corners, puts 100% of every line
on dense mesh edges, and makes every wall end a three-valent boundary vertex.
`examples/11_thesis_curve_features.py` passes 8 of 8 extremity checks on Figs
4.17–4.19, which it passed 0 of before 2026-09-14. The dense mesh covers the
whole square on all three figures.

The pipeline, stage by stage — each stage is one panel in `examples/000_testing.py`:

1. **Triangulation.** `from_boundary` samples walls and lines at the same spacing
   (`discretise_line`). `boundary_triangulation` puts a vertex at every crossing,
   welds junctions, triangulates, and **cuts** along every line
   (`mesh_unweld_edges`). A line from wall to wall splits the domain in two; a
   free line is a slit whose tip the domain wraps around.
2. **Skeleton.** Pruning and grafting run on the cut triangulation. A graft ends
   on a Delaunay vertex, on a wall or on a line; `merge_graft_targets` joins
   grafts on adjacent samples of a line, keeping a chain end.
3. **Decomposition.** Closing splits the boundary at corners, graft ends and free
   tips (`free_tip_vertices`), and the corrections of §4.2.3 add branches.
4. **Coarse mesh.** `from_polylines` welds the two sides of each line back
   together. A graft that landed on one side only leaves a pentagon, which
   `quadrangulate_polygonal_faces` resolves by carrying the seam across to a wall
   (Fig 4.20d).
5. **Dense mesh.** Densification as for any layout.

**Six defects fixed on 2026-09-15**, each first measured as a prototype:

| # | defect | where |
|---|---|---|
| 1 | The cut looked feature points up by position. A line end that is also a wall point is in the vertex list twice, and the lookup took the copy no triangle uses, so the cut edge was not a mesh edge. The cut never reached a wall landing or a junction: the line stayed a slit, its end segments uncut and its corners lost. | `boundary_triangulation` |
| 2 | Seam propagation collected valencies per component, so it never found a source and never ran. | `quadrangulate_polygonal_faces` |
| 3 | With one global source list, a pentagon holding both discrepancies of a straight line saw 2 sources and 3 corners, and was skipped. Sources are now per face, from each copy's own valency. | same, and `propagation.quadrangulate_faces` |
| 4 | Both sides of a slit join into one segment, so a free tip had only two branches and was merged away. | `free_tip_vertices`, `decomposition_polylines` |
| 5 | `merge_graft_targets` moved a tip's graft one sample inward whenever the chain ran towards the tip — both free arms of Fig 4.19. | `merge_graft_targets` |
| 6 | A vertex created by propagation counted as a source in every face, but where two strips cross it is a real corner. Also added: a strip that closes on itself is abandoned once the layout triples in size, and a face through one vertex twice is skipped. | `propagation.quadrangulate_faces`, `is_straight_through`, `quadrangulate_face` |

Each is guarded in `tests/test_pipeline.py`; eight of the new or updated tests
fail on the pre-fix source. `Skeleton.real_neighbors` only compensated for
defect 1 — removing it changes nothing once the cut works — and is kept as a
safeguard.

**Measured on 288 random inputs** — six domains × corner-to-corner, wall-to-wall,
wall-to-free, free-free, crossing pairs and bent lines × two spacings:

| | before | after |
|---|---|---|
| fallback repair | 73% | 20% |
| every line fully on dense edges | 47% | 96% |
| dense mesh with holes | 52% | 20% |
| domain corners kept | 88% | 99% |
| wall ends three-valent (R1) | 60% | 93% |
| free ends are layout vertices | 66% | 99.7% |
| crashes | 4 | 1 |

**Still open** — each measured and characterised, none fixed:

- **Folds at two-valent free tips.** A tip's second edge usually runs to a
  singular point off the line's axis, so one patch corner reaches 215–251° and
  its Coons grid folds. Fig 4.18 has 2 flipped faces, and across the sweep the
  share of dense meshes with any flipped face rose from 7% to 18.5%. Smoothing
  around the tip with the line pinned only reduces it, and rotating the partner
  node onto the line's extension fixes 3 cases of 7 and worsens one.
  `edges_to_curves` cleared it on 4.18. Fig 4.22's correction is the thesis's
  answer and is not built.
- **Poles at free tips touched by several singular faces** — the pseudo-quad of
  Fig 4.21d: valid, but not corrected to two-valent.
- **The remaining fallbacks**, 20% of the sweep: mostly a line meeting a wall at
  about 10–15°, which leaves a sliver narrower than the sampling, and crossing
  features around which a seam strip closes on itself.
- **Gentle bends** of 4–10° are not layout vertices, so the coarse edge is a
  chord across the bend and only ~60% of the line lies on edges.
  `edges_to_curves` lifts that to ~93%.
- **Thin seam strips.** Every discrepancy runs to a wall as a strip of its own.
  Moving opposite grafts to one shared sample removes them — a bent line goes
  from 29 to 21 patches, a 5×10 corner-to-corner diagonal to the 6-patch thesis
  layout — but shears the grafts. Over the random inputs it made 60 of the 116
  changed cases worse at a 45° limit. Prototyped, not adopted; the decision is
  open.
- **A closed feature** still goes through the fallback repair. Not re-measured
  since the fixes.
- **The constrained Delaunay** (Chew 1989) is still not implemented.

**How to measure a line feature.** Do not judge one by "all quads", patch counts
or a distance to the nearest edge: at an edge length of 0.5 every point is within
0.25 of some edge. Those numbers declared broken layouts a success for two
weeks. Use the columns `examples/000_testing.py` prints instead:

- holes and folds separately, since a fold inflates the area ratio;
- the domain corners kept;
- the share of each line lying ON dense edges;
- the valence of every line end.

Then look at all five stages.

Everything below is the dated log. Measurements in it before 2026-09-14 used the
blind metrics above; where a conclusion turned out to be wrong, it says so.
...
sizes. (The free diagonal's row was re-measured on 2026-09-15 at 12 faces. Its
old count described a layout that dropped both tips and covered 95.9% of the
square.)
...
`test_a_collapsed_edge_is_opened_enough_to_be_an_edge`. Since 2026-09-15 no
curve-feature input reaches case 1 any more — free tips are layout vertices — so
that test builds its own two-triangle layout.

**Update 2026-09-15: fixed.** The corner below was lost because the cut never
reached the wall (defect 1 in *Current state*), not because of the corner test.
With the cut complete, the landing corner is two copies, each an ordinary
two-valent corner. The three attempts below fought the symptom.
...
**This is what stopped the fixes reaching Rhino.** `CMD02_coarse_mesh` wires the
guides through correctly, but they arrived as two points and the cut did nothing.
(`CMD01_boundary_selection` has a separate problem: line 280 sets
...
**Correction (2026-09-14):** all-quad, but not right. The corners the diagonal
lands on were cut off, and 4.18 and 4.19 had holes — the dense mesh covered 89%
and 90% of the square. All-quad and patch counts could not see either.
...
**Correction (2026-09-15):** `mesh_unweld_edges` does cut those segments. The
cut edge handed to it was not a mesh edge, because the line end was looked up as
the duplicate vertex no triangle uses (defect 1 in *Current state*).
`real_neighbors` worked around the symptom.
...
**Correction (2026-09-14):** it was not. Both corners the diagonal lands on were
cut off, and only 12% of the diagonal lay on dense mesh edges.
...
**Superseded (2026-09-15):** R1 and R2 were not blocked by case 0. The free tips
came back once they were kept as split points (defect 4), and the wall ends once
the cut reached the walls (defect 1).
...
rule. Its "R3 drift" column is the vacuous distance test; `examples/000_testing.py`
prints the honest columns.

Status as of 2026-09-15:

| rule | requirement | status |
|---|---|---|
| R1 | extremity **on** the boundary → a three-valent boundary vertex, **no singularity** | met on every benchmark; 93% of random inputs |
| R2 | extremity **off** the boundary → exactly **one** singularity, two-valent (or a pole, corrected to two-valent by Fig 4.22) | tips are layout vertices (99.7%): two-valent when one singular face touches them, a pole when several do. Fig 4.22 is not built, and two-valent tips can fold |
| R3 | pattern aligned along / around the curve | met — 100% of each line on dense edges in the benchmarks, 96% of random inputs; a gentle bend becomes a chord |
| R4 | coarse decomposition all quads | met; the fallback repair still runs on 20% of random inputs |
| R5 | no skeleton branch crosses a feature | met — the cut |
| R6 | features combined and superimposed | met — crossings get a vertex |

And the four pipeline steps the text names:

| step | present? |
|---|---|
| points subdivide the curves, their edges **constrain** the Delaunay (Chew 1989) | **never, in any revision** — the curves are subdivided, not constrained |
| topological cut (unweld) along the curves | yes; it reaches walls and junctions since 2026-09-15 |
| seam propagation of the discrepancies → quads (Fig 4.20d) | yes. Its call was restored 2026-09-01, but it found no source until 2026-09-15 |
| correction: missed corners | yes; with the cut complete there is no slit end at a wall landing for it to fire on |
| correction: **unwanted triangles** (Fig 4.22) | exists, but **never fires at a cut** — it tests `is_vertex_on_boundary` on the coarse mesh, and `from_polylines` welds the cut shut. Fig 4.22's pole → two-valent conversion is not built |
...
**Correction (2026-09-15):** the conclusion below is wrong. They failed because
seam propagation never ran and the cut never reached the walls — not because
pentagons cannot be quadrangulated locally. With defects 1–3 fixed, "pin free
extremities" is defect 4's fix, adopted, and the pentagons are resolved by seam
propagation.
...
* **Fig 4.22's "unwanted triangle" operation**, which collapses the pole at a
  free extremity into a two-valent singularity. Since 2026-09-15 a free tip is
  two-valent when one singular face touches it and a pole when several do; only
  the second case would need this. The two-valent tips fold — see *Current
  state*.
* **A closed curve feature** goes through the fallback repair
  (`repair_polygonal_faces` → `framefield.separatrix_network.solve_non_quad_faces`) rather
