...
## 3. Layer three — the singularity as a *block*, not a joint (removed)

*`src/compas_singular/blocks.py` was removed on 2026-09-24; the feature is no longer
required.* It turned a picked point into the centre of an n-gon face with quads all round
it, instead of a pole. Its tests and its two examples (`09_block_from_point.py`,
`New approach/23_singularity_block.py`) went with it. The dual mesh (`dual_mesh`), which
turns a whole quad mesh into blocks, is separate and stays.

---

## 4. Layer four — guide lines: making a curve steer the mesh (removed)

*`src/compas_singular/guide_lines.py` was removed on 2026-09-24*, together with
`examples/FloorExample.py`, `FloorExample vault.py`, `FloorExampleGuide.py` and its test.
It was an early attempt at steering the mesh with guides: snap existing polyedges onto a
curve (`guide_line_mesh`), inflate them into bands down the middle of a row of faces
(`guide_band_mesh`), or pass the guides as curve features (`guide_feature_mesh`). Guides
are now handled by the frame field (as constraints) and by `editing/guide_chain.py` with
`smooth_guides` (attaching and smoothing an existing polyedge onto a curve).
...
### 2026-09-14/15 — "densifies" was not "works"

Every case densified, yet the layouts were wrong. Wall landings lost their corners, the
dense mesh had holes, and on Fig 4.17 only 12% of the diagonal lay on mesh edges. The
checks were blind to it: all-quad, patch counts, and a distance-to-nearest-edge test that
passes whenever edges are 0.5 apart. Six more defects, all fixed:

* the cut looked a line end up as its duplicate vertex that no triangle uses, so the cut
  never reached a wall or a junction;
* seam propagation collected its sources per component, found none, and never ran;
* a single global source list made a pentagon on a straight line see three corners;
* free tips were merged away when both sides of the slit joined into one segment;
* `merge_graft_targets` moved a tip's graft inward when the chain ran towards the tip;
* vertices created by propagation stayed sources after becoming corners. A runaway strip
  is now abandoned, and a face through one vertex twice is skipped.

Over 288 random inputs: fallback repair 73% → 20%, every line fully on mesh edges 47% →
96%, corners kept 88% → 99%, wall ends three-valent 60% → 93%. Details, and the cases
still open, are in `HOW_IT_WORKS.md` §5, *Current state*.

### Still open

* **Folds at two-valent free tips** — the tip's second edge runs off the line's axis.
* **Poles at free tips** touched by several singular faces: Fig 4.22's correction is not built.
* **The remaining fallbacks**, mostly slivers where a line meets a wall at a shallow angle.
* **Gentle bends** become chords unless `edges_to_curves` is passed.
* **A closed feature** still needs the fallback repair.
* **Chew's constrained Delaunay** is not implemented.
...
| `dual_mesh.py` | 537 | the dual of a quad mesh, **closed on the boundary** — imports nothing from Rhino, and moved to `algorithms/dual_mesh.py` on 2026-09-24 |
...
| | Tests | Covers |
|---|---|---|
| `tests/test_pipeline.py` | 4 | densification, pole survival through JSON, skeleton decomposition, lizard grammar manifoldness |
...
| Skeleton | `08` | skeleton visualisation |
| Workflow demos | `MoreControl`, `Editing QuadMesh`, `StripDelete`, `Smoothing*`, `BasicCommands` | the user-facing workflows |
...
| **2026-09-15** | curve features actually correct — six defects behind lost corners, holes and missing free tips (§4a) |
...
| `dev` | **the working branch, and what the fork publishes** — the full library: the port plus the frame field, the editing package and the research examples, all under `src/compas_singular/` |
...
> plus the machinery to make that usable in practice: headless
