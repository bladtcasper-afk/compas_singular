...
:func:`~compas_singular.framefield.mesh_quality` returns, among others:
...
1. :func:`~compas_singular.framefield.relax_mesh` -- the safe first move.
...
2. :func:`~compas_singular.datastructures.boundary_constrained_smoothing` -- for a mesh that is slack overall.
...
3. :func:`~compas_singular.datastructures.region_smoothing` -- for one bad spot.
   Only the given vertices move, and the smoothing fades out over a few rings around them, so it blends in.
4. :class:`~compas_singular.editing.GuideCurve` and :func:`~compas_singular.editing.attach_chain` --
...
5. :func:`~compas_singular.datastructures.relaxation` -- force-density relaxation with :mod:`compas_fd`,
...
:doc:`/examples/06_quad_mesh_smoothing` compares a fixed boundary, a sliding boundary and a region on the same mesh.
...
edit the coarse layout (:doc:`/examples/03_coarse_mesh_editing`),
or add and remove lines on the dense mesh (:doc:`/examples/07_quad_mesh_editing`).
